...
/**An object representing a waypoint like a stop in a ride with KPIs to be analyzed and ready to be used for forecasts*/
public class LineRideDetails {
	...
	//TODO: the unique id of the trip
		//-- trip id- that is the circulation number or so so the unique key for that - maybe daily ride key plus the start time of the trip origin to generate it naturally
	...
	/** The operation date. Sometimes schedules can leap over to the next day so we collect here one day together */
	...
	/**The natural identifier for this recurring ride.
	 * Each ride at the same time on a different day will have the same key when it is panned to stop within 100m at the same stop.
	 * */
	public String dayMeta_dailyRideStopKey;
	/**
	 * Often rides get shifted for a few minutes and the dayMeta_dailyRideStopKey cannot be used for other days.
	 * Therefore, we have here the keys which identify the nearest rides
	 * */
	...
	// the origin details and destination details are not needed and could be queried with subsequent chained queries, but they are here for convenience reasons
	 ...
	 * 	- On time = up to 5 minutes
			- Acceptable delay = 6 to 10 minutes
	...
	/**The data transmitting organization. 
	 * Does not mean the data is coming form this organization it means just who transmitted the data to iunera.
	 * */
