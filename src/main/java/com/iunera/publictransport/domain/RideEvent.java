...
public class RideEvent implements Serializable {
	...
	/**
	 * Indicator if the waypoint or stopevent is near an official bus stop.
	 * Busses may stop without official stops  e.g. to let a granny out at the doctor or similar
	 * Busses might just drive through a busstop when nobody is there without event stopping
	 * Hence, this variable indicates that the event happened near a busstop.
	 * */
	public boolean hasOfficalStopProximity;
	
	/*
	 * likely not needed
	 * The counting system used e.g. countingSystemMGElectronic */
...
//	public String countingSystemVersion;
	...
	/**
	 * The legal entity providing the data to be able to filter the data later for
	 * it
	 */
	...
	/**
	 * OPTIONAL: The logical operation date of the ride. E.g. for 1 am it might be
	 * the date before
	 */
	...
	public String uniqueVehicleId() {
		if(this.vehicleCountingSystemId!=null) return this.vehicleCountingSystemId;
		if(this.licensePlate!=null) return this.licensePlate;
		
		throw new IllegalAccessError("Vehicle does not have a unqiue ID");
	}
	/**
	 * The unique ID of the counting system installation - the unique ID of the
	 * counting system in the vehicle
	 * Serial number of the complete vehicle counting system
	 */
	...
	/** Only in the stop event 
	 * Map<DOOR, EntryExitActivity>
	 * Strings can be DOOR1..DOORn
	 * */
	...
	public RideEvent(
			EventType eventType, String eventSubtype,
			String provider, Double latitude, Double longitude, LocalDate operationDate,
			Instant timeStamp,
			String vehicleCountingSystemId, String vehicleId, String licensePlate, String line, //String ingestionPartition, 
