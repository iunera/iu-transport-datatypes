...
public class RideKeysGeneration {
	...
	/**
	 * Returns a minute based key for the ride that shall be unique in the following
	 * sense: Only one bus of one line can be scheduled to leave in in minute of a
	 * day within 110m
	 */
	public static String getRideDayKeyForStop(Double longitude, Double latitude, LocalTime depaturetime,
			String linelabel, TransportProductDTO transportProduct) {

		return getNearestTimeBucketsKeys(CoordinateFunctionStop, longitude, latitude, 3, depaturetime, linelabel,
				transportProduct, directionUNKNOWN, 1)[1];
	}

	/***
	 * generates a unique key for a stop use the planned longitude and the planned
	 * latitude.. this is important that if there is a temporary shift out of a
	 * construction work that the stop is still the sae
	 */
	public static String getKeyForStop(Double longitude, Double latitude, TransportProductDTO transportProduct,
			String direction) {
	...
	/**
	 * returns IDs containing the nearest time buckets for a ride. Often rides can
	 * be shifted by
	 * 
	 * @param precision 1 ± 2500 km 2 ± 630 km 3 ± 78 km 4 ± 20 km 5 ± 2.4 km 6 ±
	 *                  610m 7 ± 76m 8 ± 19m 9 ± 2,4 10 ± 60cm 11 ± 7,4 cm
	 */
	public static String[] getNearestTimeBucketsKeys(String coordinateFunction, Double longitude, Double latitude,
			int precision, LocalTime depaturetime, String linelabel, TransportProductDTO transportProduct,
			String direction, int bucketsizeInMinutes ) {
