...
public enum TransportProductDTO {
  ...
  public static TransportProductDTO fromString(final String name) {
    if (name.equals(HIGH_SPEED_TRAIN.name())) return HIGH_SPEED_TRAIN;
    else if (name.equals(REGIONAL_TRAIN.name())) return REGIONAL_TRAIN;
    else if (name.equals(SUBURBAN_TRAIN.name())) return SUBURBAN_TRAIN;
    else if (name.equals(SUBWAY.name())) return SUBWAY;
    else if (name.equals(TRAM.name())) return TRAM;
    else if (name.equals(BUS.name())) return BUS;
    else if (name.equals(FERRY.name())) return FERRY;
    else if (name.equals(CABLECAR.name())) return CABLECAR;
    else if (name.equals(ON_DEMAND.name())) return ON_DEMAND;
  ...
  public static Set<TransportProductDTO> fromCodes(final char[] codes) {
    ...
    for (int i = 0; i < codes.length; i++) products.add(TransportProductDTO.fromCode(codes[i]));
    return products;
  }
  ...
  public static char[] toCodes(final Set<TransportProductDTO> products) {
    ...
    int i = 0;
    for (final TransportProductDTO product : products) codes[i++] = product.code;
    return codes;
  }
